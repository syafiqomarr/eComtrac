...
public class SelectCorporateTraining extends SecBasePage {
	...
	public String getPageTitle() {
		return "page.lbl.ezbiz.selectCorporateTraining.comtracLbl";
	}
	...
	Integer available = 0;
	SSMLabel declarationChkBoxLabel;
	SSMAjaxCheckBox declarationChkBox;
	...
	public SelectCorporateTraining() {
		...
		add(new SelectCorporateTrainingForm("selectCorporateTrainingForm", getDefaultModel()));
	}

	public SelectCorporateTraining(final String trainingTrans) {
		setDefaultModel(new CompoundPropertyModel(new LoadableDetachableModel() {
			protected Object load() {
...
//				comtracFormModel.setStandardFee(robTrainingTransaction.getTrainingId().getStandardFee());
	...
	private class SelectCorporateTrainingForm extends Form implements Serializable {

		private SelectCorporateTrainingForm(String id, IModel m) {
			super(id, m);
			...
			List<RobTrainingConfig> listTraining = robTrainingConfigService.getAvailableTrainingCorp();
...
//						target.add(specialFee);
						...
						public void onClick(AjaxRequestTarget target) {
							...
							addParticipantPopup.setPageCreator(new ModalWindow.PageCreator() {
								...
								public Page createPage() {
									return new AddTrainingCorporateParticipantForm(participant, robTrainingConfig,
			...
			showParticipantFormPanel = new SSMAjaxButton("showParticipantFormPanel") {
				...
				protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
					...
					addParticipantPopup.setPageCreator(new ModalWindow.PageCreator() {
						@Override
						public Page createPage() {
							return new AddTrainingCorporateParticipantForm(null, robTrainingConfig,
									addParticipantPopup);// edit record
						}
					});
			...
			// checkbox label
			declarationChkBoxLabel = new SSMLabel("declarationChkBoxLabel", resolve("declarationComtrac.tick.declare"));
			declarationChkBoxLabel.setEscapeModelStrings(false);
			declarationChkBoxLabel.setOutputMarkupId(true);
			trainingInfo.add(declarationChkBoxLabel);
			wmcParticipant.add(declarationChkBoxLabel);

			declarationChkBox = new SSMAjaxCheckBox("declarationChkBox",
					new PropertyModel(formModel, "declarationChkBox")) {
				@Override
				protected void onUpdate(AjaxRequestTarget target) {
					if (String.valueOf(true).equals(getValue())) {
						formModel.setDeclarationChkBox(true);
						submit.setEnabled(formModel.getDeclarationChkBox());// button save enable depends on tick
																			// checkbox
					} else {
						formModel.setDeclarationChkBox(false);
						submit.setEnabled(false);
					}
					target.add(submit);
				}
			};
			trainingInfo.add(declarationChkBox);
			wmcParticipant.add(declarationChkBox);
...
//			target.add(saveAsDraft);
		...
		if (UserEnvironmentHelper.isInternalUser()) {
			...
			robTrainingTransaction.setRegistrationChannel(Parameter.COMTRAC_REGISTRATION_CHANNEL_ecomtrac);
		}
	...
	public class ComtracFormModel implements Serializable {
		...
		private Boolean declarationChkBox;
