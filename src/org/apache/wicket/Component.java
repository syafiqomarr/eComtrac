		...
		IEventSource
{
	...
	public String resolve(String key, String... params) {
...
//			e.printStackTrace();
		...
		if(key.equals(keyValue) && StringUtils.isNotBlank(paramValue)) {//mean cannot resolve
			keyValue += paramValue;
		}
...
// (WICKET-4687)
	...
	public void addOrReplaceOnClick(String clickActionKey){
		String keyValue = resolve(clickActionKey);
		AttributeModifier onClick = new AttributeModifier( "onclick", keyValue);
		try {
			remove(onClick);//avoid duplication
		} catch (Exception e) {
		}
		add(onClick);
	}
	
	public void addOrReplaceTitle(String titleKey ){
		String keyValue = resolve(titleKey);
		AttributeModifier title = new AttributeModifier( "title", keyValue);
		try {
			remove(title);//avoid duplication
		} catch (Exception e) {
		}
		add(title);
	}
