...
public class AddTrainingParticipantForm extends BaseFrame {
	...
	public AddTrainingParticipantForm(final RobTrainingParticipant robTrainingParticipant,
	...
	public AddTrainingParticipantForm(final RobTrainingParticipant robTrainingParticipant,
		...
		add(new RegisterTrainingParticipantForm("participantForm", getDefaultModel(), addParticipantPopup,
...
//			idNo.add(new AjaxEventBehavior("onKeyUp") {
//				private static final long serialVersionUID = 1L;
//
//				@Override
//				protected void onEvent(AjaxRequestTarget target) {
//					if ((!idNo.getInput().equals("") || idNo.getInput() != null) && idNo.getInput().length() == 12) {
//
//						try {
////							  dob.setDefaultModelObject(DateUtil.generateDobFromIdNo(idNo.getInput()));
//							Date dobGenerated = LlpDateUtils.generateDobFromIdNo(idNo.getInput(), "dd/MM/yyyy");
...
//			});
			...
			add(idNo);
//			final SSMTextField designation = new SSMTextField("designation");
//			designation.setRequired(true);
//			designation.add(StringValidator.maximumLength(100));
...
//			wmcParticipant.add(designation);
			...
			feeType.setOutputMarkupId(true);
			...
			AjaxFormComponentUpdatingBehavior idNoKeyup = new AjaxFormComponentUpdatingBehavior("onkeyup") { // add
				...
				protected void onUpdate(AjaxRequestTarget target) {
					...
					tempDob = null; //for handling disable input carry value for submit
...
//								dob.setDefaultModelObject(dob);
								...
								if (trainingParticipant.getRobTrainingTransaction() != null) {
									if (trainingParticipant.getRobTrainingTransaction().getLodgerId()
											.equals("SSM STAF")) {
										...
										if (Parameter.COMTRAC_FEE_TYPE_standard.equals(participantTemp.getFeeType())) {
											...
											if (Parameter.COMTRAC_FEE_TYPE_license_sec
													.equals(participantTemp.getFeeType())) {
												wmcLsNo.setVisible(true);
												wmsMemberNo.setVisible(false);
												memberNo.setDefaultModelObject("");
											} else {
						...
						target.add(dob);					
						FeedbackPanel feedbackPanel = ((AddTrainingParticipantForm) getPage()).getFeedbackPanel();
...
//						newParticipant.setDesignation(participantsubmit.getDesignation());
						...
						newParticipant.setAmount(fee);
						newParticipant.setGstAmount(gst);
						newParticipant.setAddress1(participantsubmit.getAddress1());
						newParticipant.setFeeType(participantsubmit.getFeeType());
...
//						selectedParticipant.setDesignation(participantsubmit.getDesignation());
						...
						selectedParticipant.setFeeType(participantsubmit.getFeeType());
			...
			SSMAjaxButton submitParticipantSummary = new SSMAjaxButton("submitParticipantSummary") {
				...
				protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
					...
					robTrainingTransaction
							.setAmount((robTrainingTransaction.getAmount() - selectedParticipant.getAmount()) + fee);
					robTrainingTransaction.setGstAmount(
							robTrainingTransaction.getGstAmount() - selectedParticipant.getGstAmount() + gst);
