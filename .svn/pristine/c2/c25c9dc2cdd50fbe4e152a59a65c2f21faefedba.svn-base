		...
		IGenericComponent<T>
{
	...
	private boolean isLabelReq = false;
	private boolean isShouldTrim = true;
...
//	private String prefixLabelKey;
	...
	protected void onBeforeRender() {
		...
		if(StringUtils.isNotBlank(labelKey) && !NO_LABEL.equals(getLabelKey()) ){
			String value = resolve(labelKey);
			super.setLabel(new Model(value));
    		String lblId = getId()+"Label";
    		
    		if(getParent().get(lblId)==null ){
    			if(this instanceof Button){
        			AttributeModifier buttonLbl =new AttributeModifier( "value", value);
        			add(buttonLbl);
        		}else{
        			if(isRequired()||isLabelReq()){
        				value += "<font color='red'>* </font>";
        			}
        			SSMLabel ssmLabel = new SSMLabel(lblId, value);
        			ssmLabel.setEscapeModelStrings(false); 
	    			getParent().add(ssmLabel);
        		}
	    	}
    		
        }else{
        	String msj = getId()+" NOT SPECIFY LABEL KEY";
//        	logger.info(msj);
        }
		
		if(getForm()!=null && !getForm().isAutoCompleteForm() && this instanceof TextField ){
			AttributeModifier  autocompleteOff = new AttributeModifier("autocomplete", new Model("new-password"));
			add(autocompleteOff);
		}
	}
	...
	public void setNoLabel(){
		setLabelKey(NO_LABEL);
	}

	public String getSemanticValidation() {
		return semanticValidation;
	}

	public void setSemanticValidation(String semanticValidation) {
		this.semanticValidation = semanticValidation;
	}

	public boolean isLabelReq() {
		return isLabelReq;
	}

	public void setLabelReq(boolean isLabelReq) {
		this.isLabelReq = isLabelReq;
	}
	...
	public void setShouldTrim(boolean isShouldTrim) {
		this.isShouldTrim = isShouldTrim;
	}
