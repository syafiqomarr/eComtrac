...
{
	public static Map<Session, PageAccessSynchronizer> mapAllSession = new HashMap<Session, PageAccessSynchronizer>();
	...
	public static Session get()
	{
		...
		{
			return WicketApplication.get().fetchCreateAndSetSession(RequestCycle.get());
		}
	}
	...
	public Session(Request request)
	{
		...
		mapAllSession.put(this, pageAccessSynchronizer.get());
	...
	public void ssmError(String key, String... params){
		error(WicketApplication.resolve(key, params));
	}
	
	public void ssmError(String key){
		error(WicketApplication.resolve(key));
	}
	
	public void ssmError(SSMException exception){
		String errorMsg = exception.getMessage();
		error(errorMsg);
	}
	
	public void ssmSuccess(String key, String... params){
		success(WicketApplication.resolve(key, params));
	}
	
	public void ssmSuccess(String key){
		success(WicketApplication.resolve(key));
	}
...
//		pageAccessSynchronizer.get().lockPage(pageId);
		
		
		Thread thread = Thread.currentThread();
		PageLock lock = new PageLock(pageId, thread);
//		pageAccessSynchronizer.get().locks.get().put(pageId, lock);
		
		
		IPageManager pageManager = getApplication().internalGetPageManager();
		...
		LazyInitializer<PageAccessSynchronizer>
	{
		...
		protected PageAccessSynchronizer createInstance()
		{
			...
			if (WicketApplication.exists())
			{
				timeout = WicketApplication.get().getRequestCycleSettings().getTimeout();
			}
			else
			{
			...
			PageAccessSynchronizer p =new PageAccessSynchronizer(timeout);
			return p;
