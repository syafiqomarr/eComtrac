...
public abstract class MarkupContainer extends Component implements Iterable<Component>
{
	...
	private boolean autoCompleteForm = true;
	...
	public MarkupContainer add(final Component... childs)
	{
		...
		{
			if(child instanceof FormComponent && StringUtils.isNotBlank(getPrefixLabelKey())){
				if(StringUtils.isBlank(((FormComponent)child).getLabelKey())){
					((FormComponent)child).setLabelKey(getPrefixLabelKey()+((FormComponent)child).getId());
				}
			}
		}
	...
	public String getPrefixLabelKey() {
		return prefixLabelKey;
	}

	public void setPrefixLabelKey(String prefixLabelKey) {
		this.prefixLabelKey = prefixLabelKey;
	}
	
	public void setSemanticJSValidation(MarkupContainer container,String functionName, String[] fieldToValidate, String[] fieldToValidateRules) {
		List<String> listMainFieldToValidate = Arrays.asList(fieldToValidate);
		List<String> listMainFieldToValidateRules = Arrays.asList(fieldToValidateRules);
		setSemanticJSValidation(container, functionName, listMainFieldToValidate, listMainFieldToValidateRules);
	}
	public void setSemanticJSValidation(MarkupContainer container,String functionName, List<String> listMainFieldToValidate, List<String> listMainFieldToValidateRules) {
		
		
		String jsValidation = "";
		jsValidation += "function "+functionName+"SetRule(){";
		jsValidation += "$('.ui.form')\n";
		jsValidation += ".form({\n";
		jsValidation += "fields: {\n";
		ComponentHierarchyIterator listC = container.visitChildren(FormComponent.class);
		int formToValidateCount = 0;
		
		String formId = "";
		
		for (Iterator iterator = listC.iterator(); iterator.hasNext();) {
			FormComponent formComponent = (FormComponent) iterator.next();
			int idx = listMainFieldToValidate.indexOf(formComponent.getId());
			if(idx >= 0 ){
				
				String comId = formComponent.getMarkupId();
				formId = formComponent.getForm().getMarkupId();
//				System.out.println(comId);
				if(formToValidateCount>0){
					jsValidation+= ",\n";
				}
				jsValidation+= comId+": {\n";
				jsValidation+="identifier  : '"+comId+"',\n";
				jsValidation+="rules: [\n";
				String rules[] = StringUtils.split(listMainFieldToValidateRules.get(idx),",");
				boolean isLabelReq = false;
				for (int i = 0; i < rules.length; i++) {
					...
					String labelVal = "";
					if(StringUtils.isNotBlank(formComponent.getLabelKey() ) ){
						labelVal = resolve(formComponent.getLabelKey());
					}else {
						labelVal = "information";
					}
					
					
					
					String defaultPromt = "Please enter valid "+labelVal;
					String ruleStr = rules[i];
					if(ruleStr.indexOf("match[")!=-1) {
						try {
							String fieldToMatch = StringUtils.split(rules[i],"[")[1];
							fieldToMatch = StringUtils.split(fieldToMatch,"]")[0];
							rules[i] = "match["+formComponent.getParent().get(fieldToMatch).getMarkupId()+"]";
							defaultPromt = "Please put the same value in both fields";
						} catch (Exception e) {
							e.printStackTrace();
						}
					...
					if(ruleStr.indexOf("password")!=-1) {
						try {

							//1 digit, 1 lower, 1 upper, from 6 to 20
							String PASSWORD_PATTERN  = "((?=.*[0-9])(?=.*[a-z])(?=.*[A-Z]).{6,20})";
							rules[i] = "regExp["+PASSWORD_PATTERN+"]";
							defaultPromt = "Password must contain at least one of lower case letters, upper case letters, digits and minimum 6 characters";
						} catch (Exception e) {
							e.printStackTrace();
						}
					...
					if(ruleStr.indexOf("usernameReg")!=-1) {
						try {

							String pattern  = "/^[a-zA-Z0-9_-]{5,20}$/";
							rules[i] = "regExp["+pattern+"]";
							defaultPromt = "Please enter a 5-20 Alphabet letter only ";
						} catch (Exception e) {
							e.printStackTrace();
						}
					...
					if(ruleStr.indexOf("emptyRadio")!=-1) {
						try {
							rules[i] = "emptyRadio";
							defaultPromt = "Must be selected";
						} catch (Exception e) {
							e.printStackTrace();
						}
					...
					jsValidation+="{\n";
					jsValidation+="type   : '"+rules[i]+"',\n";
					jsValidation+="prompt : '"+defaultPromt+"'\n";
					jsValidation+="}\n";
					if(rules[i].indexOf("isNotReq")==-1){
						isLabelReq = true;
					}
				}
				formComponent.setLabelReq(isLabelReq);
				
				
//				if(formComponent.getId().toLowerCase().indexOf("postcode")!=-1){
//					jsValidation+="type   : 'exactLength[5]',\n";
//				}else{
//					jsValidation+="type   : 'empty',\n";
//				}
				
//				jsValidation+="prompt : 'Please enter valid "+resolve(formComponent.getLabelKey())+"'\n";
//				jsValidation+="}\n";
		...
		jsValidation+="},\n";
		jsValidation+="inline : true,\n";
		jsValidation+="on     : 'blur'\n";
		jsValidation+="})\n";
	    jsValidation+=";\n";
	    jsValidation+="}\n";
	    ...
	    jsValidation+="\n";
		jsValidation+="function "+functionName+"(){\n";
		jsValidation+="//"+container.getMarkupId()+">>"+formId+"\n";
		jsValidation+=functionName+"SetRule();\n";
		jsValidation+="if ( $('.ui.form').form('is valid') ) {\n";
		jsValidation+="return true;\n";
		jsValidation+="}else{\n";
		jsValidation+="return false;\n";
		jsValidation+="}\n";
		jsValidation+="}\n";
	    

		jsValidation = StringUtils.replace(jsValidation, ".ui.form", "#"+formId);
		
//		System.out.println("FormBaseValidation:"+formId);
		
		
	    Label formValidator = new Label(functionName, jsValidation);
	    formValidator.setEscapeModelStrings(false); // do not HTML escape JavaScript code
	    container.addOrReplace(formValidator);
	}

	public boolean isAutoCompleteForm() {
		return autoCompleteForm;
	}

	public void setAutoCompleteForm(boolean autoCompleteForm) {
